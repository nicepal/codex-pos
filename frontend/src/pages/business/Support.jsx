...
export default function SupportPage() {
  ...
  const columns = [
    ...
    { field: 'priority', label: 'Priority', render: (r) => <Chip label={formatDisplayText(r.priority)} size="small" color={priorityColors[r.priority]} /> },
  ...
  return (
    <>
      <PageHeader title="Support" subtitle="Get help from the PosHive team" actionLabel="New Ticket" actionIcon={<Add />} onAction={() => setOpen(true)} />
      <DataTable
        columns={columns}
        rows={data?.data || []}
        loading={isLoading}
      ...
      >
        ...
        <Grid item xs={12}>
          <TextField fullWidth select label="Priority" defaultValue="medium" {...register('priority')}>
            {['low', 'medium', 'high', 'critical'].map((p) => <MenuItem key={p} value={p}>{formatDisplayText(p)}</MenuItem>)}
          </TextField>
        </Grid>
        <Grid item xs={12}>
          <TextField fullWidth select label="Category" defaultValue="general" {...register('category')}>
            {['general', 'billing', 'technical', 'inventory'].map((c) => <MenuItem key={c} value={c}>{formatDisplayText(c)}</MenuItem>)}
          </TextField>
        </Grid>
